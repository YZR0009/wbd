...
# Analysis
#    inputs
#        t -> numeric mandatory validated  >0
#        n -> numeric mandatory validated
#    outputs
#        float .GE. 0
# Happy path
#    nominal case1:  when f1(u,n) = u, integrate(1) -> 1/2
#    nominal case2:  when f2(u,n) = u**2, integrate(1) -> 1/3
#    nominal case3:  when f3(u,n) = u**6, integrate(1) -> 1/7
#    nominal case4:  when f4(u,n) = u**100, integrate(1) -> 1/101


    def test500_010_ShouldCalculateIntegrateNominal1(self):
        myT = T.TCurve(self.nominalN)
        self.assertAlmostEqual(myT.integrate(1.0, 5, myT.f1), 1/2.0, 3)

    def test500_020_ShouldCalculateIntegrateNominal2(self):
        myT = T.TCurve(self.nominalN)
        self.assertAlmostEqual(myT.integrate(1.0, 5, myT.f2), 1/3.0, 3)
                               
    def test500_030_ShouldCalculateIntegrateNominal3(self):
        myT = T.TCurve(self.nominalN)
        self.assertAlmostEqual(myT.integrate(1.0, 5, myT.f3), 1/7.0, 3)
 
    def test500_040_ShouldCalculateIntegrateNominal4(self):
        myT = T.TCurve(self.nominalN)
        self.assertAlmostEqual(myT.integrate(1.0, 5, myT.f4), 1/101.0, 3)    
